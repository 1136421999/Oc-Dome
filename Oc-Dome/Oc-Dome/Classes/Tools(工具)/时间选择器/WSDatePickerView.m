...
typedef void(^doneBlock)(NSString *, NSDate *, WSDateStyle);
...
+ (void)showDateWithStyle:(WSDateStyle)style action:(void(^)(NSString *dateString, NSDate *data))action {
    WSDatePickerView *view = [[WSDatePickerView alloc] initWithDateStyle:style CompleteBlock:^(NSString *dateString, NSDate *data, WSDateStyle style) {
        action(dateString,data);
    }];
    [view show];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    switch (self.datePickerStyle) {
        case DateStyleShowYearMonthDayHourMinute:{ // 显示年月日 时分
            if (component == 0) {
                yearIndex = row;
//                self.showYearView.text =_yearArray[yearIndex];
        ...
        case DateStyleShowYearMonthDay:{ // 显示年月日
            if (component == 0) {
                yearIndex = row;
//                self.showYearView.text =_yearArray[yearIndex];
            }
        ...
        case DateStyleShowMonthDayHourMinute:{ // 显示月日 时分
            if (component == 1) {
                dayIndex = row;
            }
            if (component == 2) {
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    switch (_datePickerStyle) {
        case DateStyleShowYearMonthDayHourMinute:{
            date = [_startDate stringWithFormat:@"yyyy-MM-dd HH:mm"];
        }break;
        case DateStyleShowMonthDayHourMinute:{
            date = [_startDate stringWithFormat:@"yyyy-MM-dd HH:mm"];
        }break;
        case DateStyleShowYearMonthDay:{
            date = [_startDate stringWithFormat:@"yyyy-MM-dd"];
        }break;
        case DateStyleShowMonthDay:{
            date = [_startDate stringWithFormat:@"yyyy-MM-dd"];
        }break;
        case DateStyleShowHourMinute:{
            date = [_startDate stringWithFormat:@"HH:mm"];
        }break;
    }
    self.doneBlock(date, _startDate, _datePickerStyle);
    [self dismiss];
}
#pragma mark - 取消按钮点击
