...
#import "HWShowBigImageView.h"

@interface HWShowBigImageView()
/** <#注释#> */
...
@end
@implementation HWShowBigImageView

+ (HWShowBigImageView *)shareInstance {
    static HWShowBigImageView *showBigImage = nil;
    if (showBigImage == nil) {
        showBigImage = [[HWShowBigImageView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    }
...
- (void)ImageReductionClose {
    ...
    [UIView animateWithDuration:0.4 animations:^{
        //背景图透明
        [weakSelf.scrollerView setBackgroundColor:[UIColor clearColor]];
        //还原缩放比例
        weakSelf.scrollerView.zoomScale = weakSelf.proportionFloat;
        //还原图片初始框架
        CGRect ARect = weakSelf.showImageView.frame;
        ARect.origin.x = weakSelf.imageOriginalRect.origin.x - weakSelf.leverlFloat;
        ARect.origin.y = weakSelf.imageOriginalRect.origin.y - weakSelf.verticalFloat;
        ARect.size.width = weakSelf.imageOriginalRect.size.width;
        ARect.size.height = weakSelf.imageOriginalRect.size.height;
        weakSelf.showImageView.frame = ARect;
        [UIView commitAnimations];
    } completion:^(BOOL finished) {
        [weakSelf.showImageView removeFromSuperview];
        weakSelf.showImageView = nil;
        [weakSelf.scrollerView removeFromSuperview];
        weakSelf.scrollerView = nil;
...
- (void)enlargeImage:(CGPoint)ponint {
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.4 animations:^{
        [weakSelf.scrollerView setZoomScale:weakSelf.scrollerView.maximumZoomScale];
    }];
    //相对边距坐标
    ...
    [UIView animateWithDuration:0.4 animations:^{
        [weakSelf.scrollerView setContentOffset:AOffsetPoint];
    }];
}
...
+ (void)showBigImage:(UIImageView *)imageView {
    HWShowBigImageView *view = [self shareInstance];
    [view showBigImage:imageView];
}
//显示图片
- (void)showBigImage:(UIImageView *)imageView {
    //添加显示视图
...
- (void)ImageAdaptCenter {
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.4 animations:^{
        [weakSelf.scrollerView setBackgroundColor:[UIColor blackColor]];
        CGRect ARect = weakSelf.showImageView.frame;
        ARect.size.width = weakSelf.showImageView.image.size.width * weakSelf.proportionFloat;
        ARect.size.height = weakSelf.showImageView.image.size.height * weakSelf.proportionFloat;
        weakSelf.showImageView.frame = ARect;
        weakSelf.showImageView.center = self.center;
    } completion:^(BOOL finished) {
        [weakSelf.showImageView setFrame:CGRectMake(0, 0, weakSelf.showImageView.image.size.width, weakSelf.showImageView.image.size.height)];
        //初始化缩放比例
        [self initScaling];
        //初始化边距
        [self initMargin];
