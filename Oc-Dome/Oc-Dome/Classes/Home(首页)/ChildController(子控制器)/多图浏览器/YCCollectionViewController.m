...
- (void)setCollectionView {
    HWWeakSelf(weakSelf)
    [self.collectionView hw_registerCell:[YCCollectionViewCell class]];
    [self setCellForRowBlock:^UICollectionViewCell * _Nonnull(NSIndexPath * _Nonnull indexPath) {
        return [weakSelf getYCCollectionViewCell:indexPath];
    } cellSizeBlock:^CGSize(NSIndexPath * _Nonnull indexPath) {
        CGFloat width = ([UIScreen mainScreen].bounds.size.width - 30)/3;
        return CGSizeMake(width, width);
    }];
}
- (UICollectionViewCell *)getYCCollectionViewCell:(NSIndexPath *)indexPath {
    YCCollectionViewCell *cell = [self.collectionView hw_dequeueReusableCell:[YCCollectionViewCell class] and:indexPath];
