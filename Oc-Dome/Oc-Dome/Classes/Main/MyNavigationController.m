...
/**
 导航栏操作类型
 */
#define NavScreenWidth [UIScreen mainScreen].bounds.size.width
#define NavScreenHeight [UIScreen mainScreen].bounds.size.height
...
-(void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    UIImageView * screentImgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, NavScreenWidth, NavScreenHeight)];
    ...
    fromViewEndFrame.origin.x = NavScreenWidth;
    ...
    if (self.navigationOperation == UINavigationControllerOperationPush) {
        ...
        UIView * nextVC = [[UIView alloc]initWithFrame:CGRectMake(NavScreenWidth, 0, NavScreenWidth, NavScreenHeight)];
        ...
        self.navigationController.view.transform = CGAffineTransformMakeTranslation(NavScreenWidth, 0);
        [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
            //toView.frame = toViewEndFrame;
            self.navigationController.view.transform = CGAffineTransformMakeTranslation(0, 0);
            screentImgView.center = CGPointMake(-NavScreenWidth/2, NavScreenHeight / 2);
    ...
    if (self.navigationOperation == UINavigationControllerOperationPop) {
        ...
        [containerView addSubview:toView];
        UIImageView * lastVcImgView = [[UIImageView alloc]initWithFrame:CGRectMake(-NavScreenWidth, 0, NavScreenWidth, NavScreenHeight)];
        ...
        [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
            screentImgView.center = CGPointMake(NavScreenWidth * 3 / 2 , NavScreenHeight / 2);
            lastVcImgView.center = CGPointMake(NavScreenWidth/2, NavScreenHeight/2);
...
- (UIImage *)screenShot {
    ...
    // 要裁剪的矩形范围
    CGRect rect = CGRectMake(0, 0, NavScreenWidth, NavScreenHeight);
...
// MARK: Pan手势识别器
- (UIScreenEdgePanGestureRecognizer *)panGestureRec {
    if (!_panGestureRec) {
...
- (void)panGestureRec:(UIScreenEdgePanGestureRecognizer *)panGestureRec {
    ...
    switch (panGestureRec.state) {
        ...
        case UIGestureRecognizerStateEnded: // 结束拖拽阶段
            [self dragEnd];
            break;
        default: // 正在拖拽阶段
...
- (void)dragging:(UIPanGestureRecognizer *)pan {
    ...
    if (offsetX < NavScreenWidth) {
        _screenshotImgView.transform = CGAffineTransformMakeTranslation((offsetX - NavScreenWidth) * 0.6, 0);
    }
...
- (void)dragEnd {
    ...
    if (translateX <= 60) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self.screenshotImgView.transform = CGAffineTransformMakeTranslation(-NavScreenWidth, 0);
